...
export async function generateMetadata(
    ...
    return generatePostMetadata(post, 'blog');
}
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
    ...
    const [post, settings, relatedPosts] = await Promise.all([
        ...
        serverApi.getPostBySlug(slug, 'blog').then(p => 
    ...
    if (!post) {
        ...
        if (uuidPattern.test(slug)) {
            ...
            if (postById && postById.slug) {
                ...
                permanentRedirect(`/blog/${postById.slug}`);
            }
        }
        notFound();
    }
                    ...
                    <div className="container-custom py-12 bg-[var(--color-background)]">
                        <div className="max-w-4xl mx-auto">
                            ...
                            {relatedPosts && relatedPosts.length > 0 && (
                                <RelatedPosts posts={relatedPosts} type="blog" />
                            )}
                        </div>
                    </div>
                </article>
